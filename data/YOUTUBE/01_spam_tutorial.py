...
df_train, df_dev, df_valid, df_test = load_spam_dataset(load_train_labels=True)
...
X_train = vectorizer.fit_transform(df_train.text.tolist()).toarray()
X_dev = vectorizer.transform(df_dev.text.tolist()).toarray()
X_valid = vectorizer.transform(df_valid.text.tolist()).toarray()
X_test = vectorizer.transform(df_test.text.tolist()).toarray()

U_x = X_train
U_L = df_train.label.values
U_l = L_train
