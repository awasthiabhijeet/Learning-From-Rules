...
do
   ...
   do
      ...
      do
         for Q in "${model_id[@]}"
         do
            nohup ./MITR.sh decoupled_"$MODE"_"$GAMMA"_"$LAMDA"_"$Q" $test_mode $EPOCHS $LR \
            $CKPT_LOAD_MODE $DROPOUT_KEEP_PROB $D_PICKLE_NAME \
            $VALID_PICKLE_NAME $U_PICKLE_NAME $GAMMA $LAMDA $USE_JOINT_f_w > $logdir/decoupled_test_"$MODE"_"$GAMMA"_"$LAMDA"_"$Q".txt &
         done
      done
   done
done
